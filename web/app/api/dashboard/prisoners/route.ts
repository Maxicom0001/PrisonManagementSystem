...
export async function GET() {
    ...
    try {
        const total = await queryOne<CountRow>("SELECT COUNT(*) AS total FROM convicts");
        const newest = await queryOne<Convict>("SELECT id, data_osadzenia, imie, nazwisko FROM convicts ORDER BY data_osadzenia ASC LIMIT 1");
        const oldest = await queryOne<Convict>("SELECT id, data_osadzenia, imie, nazwisko FROM convicts ORDER BY data_osadzenia DESC LIMIT 1");
