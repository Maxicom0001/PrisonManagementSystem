...
export default function PrisonTile() {
    ...
    // Calculate the percentage for cell blocks (assuming there's a maximum value in your data)
    // If there's no maximum value in your data, you might need to add one
    const maxCellBlocks = 25;
    const cellBlocksPercentage = data.prison.cellBlocks > 0 ? Math.min((data.prison.cellBlocks / maxCellBlocks) * 100, 100) : 0;
        ...
        <motion.div variants={itemVariants}>
            <Card className="h-full overflow-hidden flex flex-col p-0">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex items-end gap-2">
                                ...
                                {data.prison.maxCellBlocks && <span className="text-muted-foreground text-sm">of {data.prison.maxCellBlocks}</span>}
                            </div>
                            <div className="w-full bg-muted rounded-full h-2 mt-2">
                                <motion.div
                                    className="bg-green-700 h-2 rounded-full"
                                    initial={{ width: 0 }}
                                    animate={{ width: `${cellBlocksPercentage}%` }}
                        ...
                        <div className="flex justify-between items-center pt-4">
                            ...
                            <motion.div className="flex gap-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.3 }}>
                ...
                <CardFooter className="bg-green-500/5 py-2 mt-auto px-4">
                    <Button variant="ghost" disabled size="sm" className="ml-auto text-green-600"></Button>
                </CardFooter>
            </Card>
        </motion.div>
    );
                ...
                <CardContent className="pt-4 px-4 flex-1">
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="w-full bg-muted rounded-full h-2 mt-2">
                                <div className="bg-green-500/30 h-2 rounded-full w-[40%]" />
                            </div>
                        </div>
                        <div className="mt-4">
                            ...
                            <div className="w-full bg-muted rounded-full h-2 mt-2">
                                <div className="bg-green-700/30 h-2 rounded-full w-[60%]" />
                            </div>
                        </div>
                        <div className="flex justify-between items-center pt-4">
                            ...
                            <div className="flex gap-1">
                                {Array.from({ length: 8 }).map((_, i) => (
                                    <div key={i} className="w-3 h-6 bg-green-200/40 rounded-sm first:rounded-l-md last:rounded-r-md" />
                                ))}
                            </div>
                        </div>
                    </div>
