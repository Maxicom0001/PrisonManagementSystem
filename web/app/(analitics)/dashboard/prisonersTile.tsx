                ...
                <CardContent className="pt-4 px-4 flex-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div className="flex flex-col p-3 bg-red-50 dark:bg-red-950/20 rounded-lg">
                            ...
                            <div className="mt-2 flex items-center text-xs text-muted-foreground">
                                ...
                                <span>Facility at {Math.round((data.prisoners.total / Number(data.prisoners.totalCells)) * 100)}% capacity</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col p-3 bg-red-50 dark:bg-red-950/20 rounded-lg">
                            ...
                            <span className="font-medium">
                                {data.prisoners.oldest.imie} {data.prisoners.oldest.nazwisko}
                            </span>
                            <div className="flex items-center gap-1 mt-1">
                                <Badge variant="secondary" className="bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300">
                                    {new Date().getFullYear() - new Date(data.prisoners.oldest.data_osadzenia).getFullYear()} years
                                </Badge>
                                <span className="text-xs text-muted-foreground">Since {new Date(data.prisoners.oldest.data_osadzenia).getFullYear()}</span>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="font-medium">
                                {data.prisoners.newest.imie} {data.prisoners.newest.nazwisko}
                            </span>
                            <div className="flex items-center gap-1 mt-1">
                                <Calendar className="h-3 w-3" />
                                <span className="text-xs text-muted-foreground">{new Date(data.prisoners.newest.data_osadzenia).toLocaleDateString()}</span>
                            </div>
                            <Button variant="outline" size="sm" className="mt-2">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="font-medium">{data.prisoners.nextRelease.imie}</span>
                            <div className="flex items-center gap-1 mt-1">
                                <Calendar className="h-3 w-3" />
                                <span className="text-xs text-muted-foreground">{new Date(data.prisoners.nextRelease.new_date).toLocaleDateString()}</span>
                            </div>
                            <Button variant="outline" size="sm" className="mt-2 ">
